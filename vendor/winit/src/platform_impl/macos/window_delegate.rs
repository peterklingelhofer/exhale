...
impl WindowDelegate {
    ...
    pub fn set_blur(&self, _blur: bool) {
        // Stubbed out for exhale's Mac App Store build: the upstream
        // implementation calls `_CGSSetWindowBackgroundBlurRadius`, a
        // private CoreGraphics API that Apple's App Review rejects on
        // submission. exhale doesn't use winit's blur (the settings
        // window's blur is provided by NSVisualEffectView, a public
        // AppKit API), so making this a no-op removes the symbol from
        // the binary without changing user-visible behavior
    }
